...
import { RequestDetalleTutoriaDto, RequestSesionTutoriaDto, ResponseSesionTutoriaDto, SesionTutoriaService } from 'src/app/services/Nswag/sesion-tutoria.service';
import { ResponseProgramacionReservaObligatoriaDto } from '../../../services/Nswag/sesion-tutoria.service';
import { MessageService } from 'primeng/api';

@Component({
  selector: 'app-ficha-tutoria',
  templateUrl: './ficha-tutoria-obligatoria.component.html',
  styleUrls: ['./ficha-tutoria-obligatoria.component.css'],
  providers: [
    { 
...
export class FichaTutoriaObligatoriaComponent {
  ...
  ngOnInit() {
    ...
    this.activatedRoute.params
      ...
      .subscribe(result => {
        ...
        this.sesionTutoriaService.programacionActivoByCodeTutor(this.codeTutor)
            .then(programacion =>{
              this.programacionActivo = programacion;

              //Recuperrar detalle activo
              this.sesionTutoriaService.detalleProgramacionActivoByIdProgramacion(this.programacionActivo.idProgramacion!)
                  .then(detalleActivo=>{
                    this.fechaCadena =this.formatearUTCFecha(detalleActivo.fecha?.toISOString()!); 
                  })
                  .catch((err)=>{
                    console.log("No se encontro detalle activo.");
                    
                  })
              

              this.sesionTutoriaService.reservasByIdProgramacion(this.programacionActivo.idProgramacion!)
                  .then(reservas =>{
                    this.confAcademicaService.idCargaByCodeTutorado(this.tutorado.code!)
                        .then(resp =>{
                          const reservaExistente = reservas.find(reserva => reserva.idCargaTutoria === resp.idCarga );
                          //Recuperar idSesion Tutoria
                          this.sesionTutoriaService.sesionByIdReserva(reservaExistente?.idReserva!)
                          .then(resp=>{
    ...
    if(this.formFichaSesionTutoria.valid)
    {
      ...
      this.sesionTutoriaService.reservasByIdProgramacion(this.programacionActivo.idProgramacion!)
          .then(reservas =>{
            ...
            this.confAcademicaService.idCargaByCodeTutorado(this.tutorado.code!)
            .then(resp =>{
              this.idCarga = resp.idCarga!;

              const reservaExistente = reservas.find(reserva => reserva.idCargaTutoria == this.idCarga );
              ...
              .catch((errr)=>{
                console.log("La sesion no fue encontrada.");
                //Guardar ficha sesion de tutoria 
                let nuevaSesion = new RequestSesionTutoriaDto();
                nuevaSesion.idReserva = reservaExistente?.idReserva;
                ...
                //nuevaSesion.fechaReunion = this.programacionActivo.fechaOficial;
                nuevaSesion.hora = reservaExistente?.horaTutoria;
